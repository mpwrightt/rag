...
async def lifespan(app: FastAPI):
    ...
    This function initializes database connections, graph databases, and other resources
    when the application starts, and gracefully closes them when the application stops.
    It is designed to be used with FastAPI's `lifespan` parameter.
...
async def get_or_create_session(request: ChatRequest) -> str:
    ...
    If the request provides a session ID, this function validates it. If no session ID is
    provided or if the provided ID is invalid, a new session is created and its ID is
    returned.

    Args:
        request: The chat request containing session information.
...
async def get_conversation_context(
    ...
    This function fetches the last `max_messages` from the specified session to provide
    context for the agent's next response.

    Args:
        session_id: The ID of the session to retrieve context from.
...
def extract_tool_calls(result) -> List[ToolCall]:
    """
    Extracts tool call information from the result of a Pydantic AI agent run.

    This function inspects the agent's output messages to identify and parse
    any tool calls that were made during the run.

    Args:
        result: The result object from a Pydantic AI agent execution.

    Returns:
        A list of `ToolCall` objects representing the tools that were called.
    """
    tools_used = []
...
async def save_conversation_turn(
    ...
    """
    Saves a complete user-assistant conversation turn to the database.

    This function logs both the user's message and the assistant's response to ensure
    the conversation history is preserved.

    Args:
        session_id: The ID of the session to which the turn belongs.
        user_message: The message sent by the user.
        assistant_message: The response generated by the assistant.
        metadata: Optional dictionary of metadata to store with the messages.
    """
    # Save user message
    await add_message(
...
def convert_chunks_to_sources(chunks: List[ChunkResult]) -> List[SourceResult]:
    """
    Converts a list of ChunkResult objects into a list of SourceResult objects.

    This function is used to transform the detailed chunk information from the backend
    into a simpler format suitable for the frontend, while also removing duplicates.

    Args:
        chunks: A list of `ChunkResult` objects from a search operation.

    Returns:
        A list of `SourceResult` objects, formatted for frontend consumption.
    """
    sources = []
...
async def build_live_graph_payload(
    ...
    """
    Constructs a payload for rendering a live knowledge graph.

    This function analyzes the tools used in a conversation turn to build a relevant
    knowledge graph visualization. It prioritizes explicit graph traversal tool calls
    but can fall back to searching for entities in the user's message.

    Args:
        message: The user's message.
        tools_used: A list of tools called during the agent's run.
        max_expand_entities: The maximum number of entities to expand from a search.
        default_depth: The default depth for graph traversal.

    Returns:
        A dictionary representing the graph payload (nodes and edges), or None.
    """
    try:
...
async def execute_agent(
    ...
    """
    Executes the main RAG agent with a user's message.

    This function orchestrates the agent's execution, including context retrieval,
    running the agent, and saving the conversation turn.

    Args:
        message: The user's input message.
        session_id: The ID of the current session.
        user_id: The optional ID of the user.
        save_conversation: A flag indicating whether to save the conversation.

    Returns:
        A tuple containing the agent's string response, a list of tools used,
        and a list of source results.
    """
    try:
...
async def health_check():
    ...
    This endpoint checks the status of the primary database and the graph database
    to ensure they are connected and responsive.
...
async def chat(request: ChatRequest):
    ...
    This endpoint processes a user's message, executes the RAG agent, and returns
    a complete response, including the agent's message, sources, and any tools used.

    Args:
        request: A `ChatRequest` object containing the user's message and session info.

    Returns:
        A `ChatResponse` object with the agent's full response.
    """
    try:
...
async def chat_stream(request: ChatRequest):
    ...
    This endpoint allows for real-time, incremental updates as the agent processes
    the request. It streams text deltas, tool calls, and other metadata to the client
...
async def search_vector(request: SearchRequest):
    ...
    This endpoint uses the vector search tool to find documents or chunks that are
    semantically similar to the query.

    Args:
        request: A `SearchRequest` object containing the query and other parameters.

    Returns:
        A `SearchResponse` containing the vector search results.
    """
    try:
...
async def search_graph(request: SearchRequest):
    """
    Performs a search on the knowledge graph.

    This endpoint queries the knowledge graph to find facts, entities, and relationships
    related to the user's query.

    Args:
        request: A `SearchRequest` object containing the query.

    Returns:
        A `SearchResponse` containing the graph search results.
    """
    try:
...
async def search_hybrid(request: SearchRequest):
    ...
    This endpoint uses a hybrid approach to leverage both semantic similarity and
    exact keyword matching for more comprehensive search results.

    Args:
        request: A `SearchRequest` object containing the query and other parameters.

    Returns:
        A `SearchResponse` containing the hybrid search results.
    """
    try:
...
async def list_documents_endpoint(
    ...
    """
    Lists the documents available in the system.

    This endpoint supports pagination through `limit` and `offset` parameters, allowing
    clients to browse through the collection of documents.
...
async def delete_document_endpoint(document_id: str):
    ...
    This endpoint removes a document and all its associated data, such as chunks
    and embeddings, from the database.
...
def _extract_text_from_document(doc: Dict[str, Any]) -> Optional[str]:
    """
    Performs a best-effort extraction of textual content from a document dictionary.

    This function searches through various common fields and structures within the
    document object to find and return a string of its main text content.

    Args:
        doc: A dictionary representing a document.

    Returns:
        The extracted text as a string, or None if no text could be found.
    """
...
async def get_document_endpoint(document_id: str):
    """
    Retrieves a complete document, including its chunks.

    This endpoint is useful for previews or when detailed information about a
    document and its constituent parts is needed.

    Args:
        document_id: The UUID of the document to retrieve.

    Returns:
        The full document object, including a list of its chunks.
    """
    try:
...
async def get_document_content_endpoint(document_id: str, variant: str):
    """
    Returns the raw textual content of a document.

    This endpoint provides access to the text of a document in different formats,
    which is useful for previews, downloads, or other content-based operations.

    Args:
        document_id: The UUID of the document.
        variant: The desired content format (e.g., 'text', 'markdown').

    Returns:
        A `Response` object containing the raw text content.
    """
    allowed = {
...
async def upload_document(file: UploadFile = File(...)):
    """
    Uploads a document for ingestion into the system.

    This endpoint handles file uploads, validates them, and initiates the ingestion
    pipeline to process and store the document, its chunks, and its graph data.

    Args:
        file: The `UploadFile` object representing the file to be ingested.

    Returns:
        A dictionary with the results of the ingestion process.
    """
...
async def get_session_info(session_id: str):
    ...
    This endpoint returns metadata and other details associated with a given
    session ID.
...
async def list_available_integrations():
    """
    Lists the available third-party integrations.

    This endpoint provides information about which external services (like Google Drive,
    Dropbox, etc.) are supported for document importation.

    Returns:
        A list of available integrations and their properties.
    """
...
async def get_integration_auth_config(service_name: str):
    """
    Gets the OAuth configuration for a specific integration service.

    This endpoint provides the necessary details for a client-side OAuth flow,
    such as the client ID, authorization URL, and required scopes.

    Args:
        service_name: The name of the integration service.

    Returns:
        A dictionary containing the OAuth configuration.
    """
    if not INTEGRATIONS_AVAILABLE:
...
async def store_integration_token(service_name: str, request: Request):
    """
    Stores a user's OAuth token for an integration service.

    After a user completes an OAuth flow on the client side, this endpoint
    receives the access token and stores it for future use.
    ...
    Returns:
        A confirmation message and a session ID for the integration.
    """
    if not INTEGRATIONS_AVAILABLE:
        raise HTTPException(status_code=503, detail="Integrations not available")
...
async def list_integration_documents(
    ...
    """
    Lists documents from a third-party integration service.

    This endpoint allows browsing files and folders within an external service
    like Google Drive or Dropbox.

    Args:
        service_name: The name of the integration service.
        session_id: The integration session ID.
        folder_id: The optional ID of the folder to list documents from.

    Returns:
        A list of documents and folders from the integration.
    """
    if not INTEGRATIONS_AVAILABLE:
        raise HTTPException(status_code=503, detail="Integrations not available")
...
async def import_documents_from_integration(
    ...
    """
    Imports selected documents from a third-party integration.

    This endpoint triggers the importation and ingestion of documents that a user
    has selected from an external service.

    Args:
        service_name: The name of the integration service.
        request: The request object containing the list of document IDs to import.

    Returns:
        A summary of the import process, including successful and failed documents.
    """
    if not INTEGRATIONS_AVAILABLE:
        raise HTTPException(status_code=503, detail="Integrations not available")
...
async def api_get_real_time_metrics():
    """
    Retrieves real-time analytics metrics.

    This endpoint provides a snapshot of current system activity, such as active
    sessions and recent message counts.

    Returns:
        A `RealTimeMetrics` object with the latest metrics.
    """
...
async def api_get_chat_metrics(days: int = 7):
    """
    Retrieves aggregated chat metrics over a specified period.

    This endpoint provides statistics about chat activity, such as total messages
    and unique users, over the last N days.

    Args:
        days: The number of days to include in the aggregation.

    Returns:
        A `ChatMetrics` object with the aggregated chat statistics.
    """
...
async def api_get_document_usage():
    ...
    This endpoint provides insights into how documents are being used, such as
    which are most frequently referenced.

    Returns:
        A `DocumentUsageStats` object with document usage data.
    """
...
async def api_get_trending_searches(days: int = 7, limit: int = 10):
    ...
    This endpoint identifies the most popular search queries over a specified
    period, which can be used to understand user interests.

    Args:
        days: The number of days to consider for trending searches.
        limit: The maximum number of trending searches to return.

    Returns:
        A dictionary containing a list of trending search queries.
    """
...
async def api_get_user_engagement(user_id: Optional[str] = None, days: int = 30):
    ...
    This endpoint can provide engagement metrics for a specific user or for all
    users in the system.

    Args:
        user_id: An optional user ID to filter the metrics for.
        days: The number of days to include in the metrics calculation.

    Returns:
        A `UserEngagementMetrics` object with the engagement data.
    """
...
async def api_get_dashboard(days: int = 7, user_id: Optional[str] = None):
    """
    Retrieves a combined set of analytics data for a dashboard view.

    This endpoint aggregates various metrics, including real-time, chat, document,
    and user engagement data, into a single response suitable for a dashboard.

    Args:
        days: The number of days to cover in the aggregated metrics.
        user_id: An optional user ID to filter engagement metrics.

    Returns:
        An `AnalyticsDashboardResponse` object with a comprehensive set of metrics.
    """
...
async def generate_questions(collection_id: Optional[str] = None, limit: int = 6):
    ...
    This endpoint uses an AI model to generate insightful questions that can be
    asked about the documents in the system, either for a specific collection or
    across all documents.

    Args:
        collection_id: An optional ID of a document collection to focus on.
...
async def clear_question_cache():
    """
    Clears the cache for the question generator.

    This is useful for ensuring that newly generated questions reflect the latest
    document content.
...
async def debug_questions():
    """
    A debug endpoint for checking the status of document and question generation.

    This endpoint provides statistics about the number of documents and chunks,
    and shows whether the question generator is available.

    Returns:
        A dictionary with debugging information.
    """
    try:
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    This function ensures that any unexpected error in the application is caught
    ...
    Returns:
        An `ErrorResponse` object detailing the error.
    """
